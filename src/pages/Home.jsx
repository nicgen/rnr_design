    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <section className="relative h-[85vh] w-full flex-shrink-0 overflow-hidden">
        ...
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-center px-4">
          <h1 className="text-7xl md:text-9xl font-black text-white uppercase leading-none mb-4 tracking-tighter">
            TOUS NORMANDS,<br />TOUS RNR!
          </h1>
          <p className="text-white text-xl md:text-2xl font-medium max-w-3xl">
            Persévérance, et force brute, le RNR avance !
          </p>
        </div>

        {/* Matchbar sticky at bottom of hero */}
        <div className="absolute bottom-0 left-0 right-0 z-20 bg-black/80 backdrop-blur-md border-t border-white/10 hidden md:block">
          <div className="max-w-7xl mx-auto flex items-stretch h-24">
            <div className="flex items-center gap-8 px-8 border-r border-white/10 relative">
              <span className="absolute top-2 left-6 text-[8px] font-black uppercase bg-primary text-white px-2 py-0.5 tracking-widest">PROCHAIN MATCH</span>
              <div className="flex gap-4 items-center">
                <div className="text-center"><p className="text-xl font-black text-white">11</p><p className="text-[8px] uppercase font-bold text-white/60">Jours</p></div>
                <div className="text-center"><p className="text-xl font-black text-white">08</p><p className="text-[8px] uppercase font-bold text-white/60">Hrs</p></div>
                <div className="text-center"><p className="text-xl font-black text-white">00</p><p className="text-[8px] uppercase font-bold text-white/60">Min</p></div>
              </div>
            </div>
            
            <div className="flex-1 flex items-center justify-center gap-6 px-8">
              <div className="flex items-center gap-3">
                <img src={logoRNR} alt="Rouen" className="w-10 h-10" />
                <span className="font-black text-xl italic uppercase text-white">Rouen</span>
              </div>
              <span className="font-black text-xl italic text-white/40">VS</span>
              <div className="flex items-center gap-3">
                <span className="font-black text-xl italic uppercase text-white">Massy</span>
                <PlaceholderImage className="w-10 h-10 rounded-full border border-white/20" />
              </div>
              <button className="bg-primary text-white font-black uppercase px-6 py-3 ml-8 hover:bg-white hover:text-primary transition-all flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">confirmation_number</span> RÉSERVER
              </button>
            </div>

            <div className="flex items-stretch border-l border-white/10">
              <div className="px-8 flex flex-col justify-center border-r border-white/10">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] text-white/40 font-bold uppercase">28 MARS</span>
                  <span className="material-symbols-outlined text-[10px] text-white/40">location_on</span>
                </div>
                <p className="text-[10px] font-black text-white/60 uppercase mb-1 leading-none">DÉPLACEMENT</p>
                <p className="text-sm font-black text-white uppercase leading-none">NIORT RC</p>
              </div>
              <div className="px-8 flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] text-white/40 font-bold uppercase">10 AVRIL</span>
                  <span className="material-symbols-outlined text-[10px] text-white/40">home</span>
                </div>
                <p className="text-[10px] font-black text-white/60 uppercase mb-1 leading-none">RÉCEPTION</p>
                <p className="text-sm font-black text-white uppercase leading-none">CHAMBÉRY</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* DASHBOARD SECTION: STATS & RÉSULTATS */}
      <section className="bg-slate-50 py-24 w-full flex-shrink-0 border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            
            {/* Colonne Gauche: POSITION AU CLASSEMENT */}
            <div className="flex flex-col items-center justify-center p-8 bg-white border-2 border-slate-100 shadow-sm rounded-xl">
              <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-4">Position</h3>
              <div className="relative">
                <span className="text-[12rem] font-black leading-none italic tracking-tighter text-black select-none">#7</span>
                <div className="absolute -bottom-4 right-0 bg-primary text-white px-4 py-2 font-black italic transform skew-x-[-15deg]">
                  <span className="inline-block skew-x-[15deg]">SAISON 25/26</span>
                </div>
              </div>
              <p className="mt-8 text-xs font-bold uppercase tracking-widest text-slate-400">Classé sur 14 équipes</p>
            </div>

            {/* Colonne Centre: DERNIERS RÉSULTATS */}
            <div className="flex flex-col gap-4">
              <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400 mb-2">Derniers Résultats</h3>
              <div className="flex flex-col gap-3">
                {[
                  ...
                  <div key={i} className="bg-white border border-slate-100 p-4 flex items-center justify-between shadow-sm">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-slate-400 leading-none mb-1">{res.j}</span>
                      <span className="text-[8px] font-bold text-slate-300">{res.date}</span>
                    </div>
                    <div className="flex-1 flex justify-center items-center gap-3 text-sm font-black italic">
                      <span className={`uppercase w-16 text-right truncate ${res.home === 'ROUEN' ? 'text-primary' : ''}`}>{res.home}</span>
                      <span className={`${res.win ? 'bg-black text-white' : 'bg-slate-100 text-slate-400'} px-2 py-1 text-[10px] min-w-[50px] text-center transform skew-x-[-15deg]`}>
                        <span className="inline-block skew-x-[15deg]">{res.score}</span>
                      </span>
                      <span className={`uppercase w-16 text-left truncate ${res.away === 'ROUEN' ? 'text-primary' : ''}`}>{res.away}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Colonne Droite: FORME & POINTS */}
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-4">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Historique</h3>
                <div className="flex gap-2">
                  {['V', 'V', 'D', 'D', 'V'].map((f, i) => (
                    <div key={i} className={`w-10 h-10 flex items-center justify-center font-black rounded-lg ${f === 'V' ? 'bg-green-500 text-white' : 'bg-red-500 text-white shadow-inner opacity-80'}`}>
                      {f}
                    </div>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-6 border-2 border-slate-100 shadow-sm rounded-xl text-center">
                  <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Points Marqués</p>
                  <p className="text-4xl font-black italic leading-none">468</p>
                </div>
                <div className="bg-white p-6 border-2 border-slate-100 shadow-sm rounded-xl text-center">
                  <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Points Concédés</p>
                  <p className="text-4xl font-black italic leading-none text-slate-300">382</p>
                </div>
              </div>

              <div className="bg-black text-white p-6 rounded-xl flex items-center justify-between">
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest opacity-50 mb-1">Différence</p>
                  <p className="text-3xl font-black italic">+86</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] font-black uppercase tracking-widest opacity-50 mb-1">Points Class.</p>
                  <p className="text-3xl font-black italic text-primary">56</p>
                </div>
              </div>
            </div>
