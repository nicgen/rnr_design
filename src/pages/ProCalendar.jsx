  ...
  return (
    <div className="min-h-screen bg-white">
      <PageHero
        ...
        subtitle="Suivez le parcours des Lions en Nationale. Résultats, prochaines rencontres et billetterie."
        bgImage={heroImg}
          ...
          {matches.map((match, i) => (
            ...
            >
              <div className="flex flex-col items-center md:items-start w-full md:w-32 mb-4 md:mb-0">
                ...
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">{match.date}</span>
              </div>
              ...
              <div className="w-full md:w-40 flex justify-center md:justify-end">
                {match.score ? (
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-300">Match Terminé</span>
                ) : match.home === "Rouen" ? (
                  <button className="bg-black text-white px-6 py-2 text-xs font-black uppercase tracking-widest hover:bg-primary transition-all -skew-x-12">
                    <span className="inline-block skew-x-12">Billetterie</span>
                  </button>
                ) : (
                  <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-300 italic">Extérieur</span>
                )}
              </div>
            </div>
          ))}
