...
export default function SectionNav({ links, rightElement, isDark = false }) {
  ...
  const wrapperClass = isDark
    ? "flex flex-col lg:flex-row justify-between lg:items-end gap-4 border-t border-white/10"
    : "flex flex-col lg:flex-row justify-between lg:items-end gap-4 border-b-2 border-slate-200 mb-12";
          ...
          if (link.external) {
            return (
              <a
                ...
                className={`pb-4 uppercase font-black text-sm lg:text-base border-b-4 border-transparent transition-colors whitespace-nowrap flex items-center gap-1 ${inactiveClass}`}
              >
          ...
          return (
            <Link
              ...
              className={`pb-4 uppercase font-black text-sm lg:text-base border-b-4 transition-colors whitespace-nowrap ${
                isActuallyActive ? "border-primary text-primary" : inactiveClass
              }`}
            >
              {link.label}
            </Link>
